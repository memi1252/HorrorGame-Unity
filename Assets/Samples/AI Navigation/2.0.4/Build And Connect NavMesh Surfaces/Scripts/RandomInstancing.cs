...
namespace Unity.AI.Navigation.Samples
{
    ...
    public class RandomInstancing : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                var go = Instantiate(m_Prefab, Vector3.zero, Quaternion.identity, parent);
                go.SetActive(false);
        ...
        private void Update()
        {
            if (trackedAgent.remainingDistance > 0.1f) UpdateInstances();
        }
        ...
        private int UpdateTileInstances(int i, int j)
        {
            ...
            var count = Math.Min(m_InstancesPerTile, m_PoolSize - m_Used);
